...
def detect_1d_cfar(R_minus, guard_cells=2, training_cells=4, pfa=1e-3, tolerance=5):
  """
...
##############################
  # 打印初始峰值
  # print(f"Initial peak (mode): {peak}")
  
  # # 绘制初始峰值分布直方图
  # plt.figure(figsize=(10, 6))
  # plt.hist(peaksArray, bins=30, alpha=0.7, color='blue')
  # plt.axvline(x=peak, color='red', linestyle='--', linewidth=2, label=f'Mode: {peak}')
  # plt.title('Initial Distribution of Peaks')
...
##############################
  ...
  # 检查并更新与众数相差显著的峰值
  for i, (m, n_peak) in enumerate(peaks):
    # 计算当前峰值与众数的差值
    diff = abs(n_peak - peak)
    
    # 如果差值大于容差，则在容差范围内重新执行CFAR检测
    if diff > tolerance:
      # 获取当前帧的数据
      frame_data = np.mean(np.abs(R_minus[m, :, :]), axis=0)
      
      # 确定搜索范围，确保不超出数组边界
      start_idx = max(training_cells + guard_cells, peak - tolerance)
      end_idx = min(len(frame_data) - training_cells - guard_cells, peak + tolerance + 1)
      
      # 在容差范围内执行CFAR检测
      detection_map = np.zeros(end_idx - start_idx, dtype=bool)
      ...
      # 在容差范围内执行CFAR检测
      for n_idx, n in enumerate(range(start_idx, end_idx)):
        # 定义被检测单元(CUT)
        cut = frame_data[n]
        ...
        # 比较被检测单元与阈值
        if cut > threshold:
          detection_map[n_idx] = True
      
      # 在检测结果中寻找局部极大值
      detection_data = frame_data[start_idx:end_idx] * detection_map
      if np.any(detection_map):
        ...
        if len(peak_indices) > 0:
          # 寻找最强峰值
          peak_values = detection_data[peak_indices]
          strongest_idx = np.argmax(peak_values)
          new_peak = start_idx + peak_indices[strongest_idx]
          peaks[i] = (m, new_peak)
        else:
          # 如果未找到峰值，使用容差范围内的最大值
          new_peak = start_idx + np.argmax(frame_data[start_idx:end_idx])
          peaks[i] = (m, new_peak)
      else:
        # 如果未找到峰值，使用容差范围内的最大值
        new_peak = start_idx + np.argmax(frame_data[start_idx:end_idx])
...
##############################
  # # 获取更新后的峰值数组
  # updated_peaksArray = np.array(peaks)[:, 1]
  
  # # 打印更新后的峰值统计
  # print(f"After update - Peak (mode): {peak}")
  # print(f"After update - Min peak: {np.min(updated_peaksArray)}")
  # print(f"After update - Max peak: {np.max(updated_peaksArray)}")
  # print(f"After update - Mean peak: {np.mean(updated_peaksArray)}")
  # print(f"After update - Std dev: {np.std(updated_peaksArray)}")
  
  # # 绘制更新前后的峰值分布对比直方图
  # plt.figure(figsize=(12, 6))
  # plt.hist(peaksArray, bins=30, alpha=0.5, color='blue', label='Before Update')
  # plt.hist(updated_peaksArray, bins=30, alpha=0.5, color='green', label='After Update')
  # plt.axvline(x=peak, color='red', linestyle='--', linewidth=2, label=f'Mode: {peak}')
  
  # # 添加容差范围标记
  # plt.axvspan(peak-tolerance, peak+tolerance, alpha=0.2, color='red', label=f'Tolerance Range (±{tolerance})')
  
  # plt.title('Comparison of Peak Distributions Before and After Update')
  # plt.xlabel('Range Bin')
  # plt.ylabel('Frequency')
  # plt.legend()
  # plt.grid(True, alpha=0.3)
...
def extract_roi(R_minus, peaks):
  """
  围绕每帧各自的峰值提取感兴趣区域(ROI)
  ...
  """
  M, K, N = R_minus.shape
  R_hat = np.zeros((M, K, 8), dtype=R_minus.dtype)
  
  # 为每一帧提取ROI
  for i, (m, peak) in enumerate(peaks):
    # 确保ROI在有效范围内
    if (peak - 4 < 0):
      R_hat[m, :, :] = R_minus[m, :, 0:8]
    elif (peak + 4 > N):
      R_hat[m, :, :] = R_minus[m, :, N-8:N]
    else:
      R_hat[m, :, :] = R_minus[m, :, peak-4:peak+4]
  
  return R_hat

def visualize_fft_result(R, R_minus, frame_idx=0, title="2D-FFT Result", figsize=(15, 15), peaks=None, R_hat=None):
  """
  ...
  for idx in frame_indices:
    ...
    if peaks is not None:
      # 查找当前帧的峰值
      frame_peak = None
      for m, n_peak in peaks:
        if m == idx:
          frame_peak = n_peak
          break
      
      if frame_peak is not None:
        # 在每个方位角位置标记峰值
        for k in range(fft_mag2.shape[0]):
          axes[1].plot(frame_peak, k, 'rx', markersize=8)
        
        # 添加峰值位置的文本标注
        axes[1].text(frame_peak + 5, fft_mag2.shape[0]//2, f'Peak: {frame_peak}', 
...
def raw2Rhat(r, visualize=False, frame_to_visualize=0, tolerance=5):
  """
  ...
  # 步骤3: 使用1D-CFAR检测峰值
  peak, peaks = detect_1d_cfar(R_minus, tolerance=tolerance)

  # 步骤4: 提取每帧峰值周围的ROI
  R_hat = extract_roi(R_minus, peaks)

  # 如果需要则可视化FFT结果
  if visualize:
    visualize_fft_result(R, R_minus, frame_idx=frame_to_visualize, 
                        title="2D-FFT Result", peaks=peaks, R_hat=R_hat)
